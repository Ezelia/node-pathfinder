...
static coord_t adjustInDirection (coord_t c, int dir)
{
	...
	switch ((dir + 65536) % 8) {
	case 0: return (coord_t) {c.x, c.y - 1}; //N
	case 1: return (coord_t) {c.x + 1, c.y - 1}; //NE
	case 2: return (coord_t) {c.x + 1, c.y }; //E
	case 3: return (coord_t) {c.x + 1, c.y + 1}; //SE
	case 4: return (coord_t) {c.x, c.y + 1}; //S
	case 5: return (coord_t) {c.x - 1, c.y + 1}; //SW
	case 6: return (coord_t) {c.x - 1, c.y}; //W
	case 7: return (coord_t) {c.x - 1, c.y - 1}; //NW
	}
	return (coord_t) { -1, -1 };
}
